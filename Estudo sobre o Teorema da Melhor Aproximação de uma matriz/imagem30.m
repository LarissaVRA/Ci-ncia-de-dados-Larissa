...
% Gráficos (a) valores singulares e (b) soma cumulativa
% =========================================================
sigma = diag(S);
cumSigma = cumsum(sigma)/sum(sigma);

r_marks = [10 20 40 80 160 320 640];  % mesmos r da aproximação
...
semilogy(sigma,'k')
hold on
semilogy(r_marks, sigma(r_marks), 'ro','MarkerFaceColor','r')
hold off
xlabel('r')
...
% --- (b) Soma cumulativa normalizada ---
subplot(1,2,2)
plot(cumSigma,'k')
hold on
plot(r_marks, cumSigma(r_marks), 'ro','MarkerFaceColor','r')
